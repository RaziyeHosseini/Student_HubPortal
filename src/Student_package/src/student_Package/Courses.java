...
public class Courses {
	 ...
	 * <p>
	 * This method returns the numeric grade points for the provided grade (A=4,
	 * B=3, C=2, D=1, F=0). If the course is honors and the grade is not an F, one
	 * bonus point is added. This method is used by {@link GPACalculator} when
	 ...
	 *
	 * This value is used when listing a student's courses in the UI (for example,
	 * in {@link LoginApp#showStudentDashboard}).
	 *
	 ...
	 *
	 * The credit hours are used by {@link GPACalculator} to compute the weighted
	 * GPA (gradePoints * creditHours).
	 *
	 * @return credit hours (positive integer, default 3 when not specified in
	 *         input)
	 */
	...
	/**
	 * Equality is based on course name (case-insensitive), honors status, and
	 * credit hours. This ensures consistent behavior when using Courses as keys in
	 * maps (for example in {@link Student} courseGrades map).
	 *
	...
	public boolean equals(Object obj) {
		...
		return this.creditHours == other.creditHours && this.isHonors == other.isHonors
				&& (this.courseName == null ? other.courseName == null
						: this.courseName.equalsIgnoreCase(other.courseName));
	}

	/**
	 * Compute a hash code consistent with {@link #equals(Object)} so this object
	 * can be used reliably as a key in hash-based collections.
	 *
	 * @return hash code
	 */
	@Override
	...
	/**
	 * String representation used for debugging and display. Includes honors marker
	 * and credit hours.
	 *
